...
same names, but the labels and the text are in English and the png files are
written to a folder of their own (`figures/en/<year>/` instead of
`figures/<year>/`), so that the two language versions never overwrite each
other. Titles, subtitles and sources are text above the figure, not part of the
figure itself.
